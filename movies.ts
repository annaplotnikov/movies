// movies.ts
// Movie Catalog (Posters) на TypeScript
...
class MovieCatalog {
    ...
    constructor(dataFile: string = 'movies.json') {
    ...
    private load(): void {
        try {
            if (fs.existsSync(this.dataFile)) {
                ...
                const data = JSON.parse(raw) as MovieData;
    ...
    add(title: string, year: number, rating: number, poster: string, description: string = ''): Movie {
...
function main(): void {
    ...
    if (args.length === 0 || args[0] === 'help') {
        console.log(`Использование: ts-node movies.ts <команда> [опции]
